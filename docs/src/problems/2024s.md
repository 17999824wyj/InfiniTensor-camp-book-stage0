...
- [Q \& A](#q--a)
  - [目录](#目录)
  - [1. 除了 cpp 和 Rust 还需要什么专业基础嘛？](#1-除了-cpp-和-rust-还需要什么专业基础嘛)
  - [2. 直播有回放吗？](#2-直播有回放吗)
  - [3. C++ 或者 Rust 对后续专业阶段和项目阶段有什么影响吗，比如某一方向要求是 Rust](#3-c-或者-rust-对后续专业阶段和项目阶段有什么影响吗比如某一方向要求是-rust)
  - [4. 关于 InfiniTensor 拉取](#4-关于-infinitensor-拉取)
  - [5. InfiniLM 支持问题](#5-infinilm-支持问题)
  - [6. 关于导学阶段安排](#6-关于导学阶段安排)
  - [7. 关于后续基础阶段内容问题](#7-关于后续基础阶段内容问题)
  - [8. 关于 InfiniLM 运行问题](#8-关于-infinilm-运行问题)
  - [9. 关于社区平台的课程录播问题](#9-关于社区平台的课程录播问题)
  - [10. 这个训练营和编译器有啥关系？](#10-这个训练营和编译器有啥关系)
  - [11. 后续的 AI 编译器主要是做 cuda 后端的吗？](#11-后续的-ai-编译器主要是做-cuda-后端的吗)
  - [12. 后期的编译器项目，是在 InfiniTensor 还是 RefactorGraph 基础上做的？](#12-后期的编译器项目是在-infinitensor-还是-refactorgraph-基础上做的)
  - [13. 训练营的 C++ 使用什么标准？](#13-训练营的-c-使用什么标准)
  - [14. 有没有正式课程的文档？对于有 Rust 经验的学生可以先看看正式课程](#14-有没有正式课程的文档对于有-rust-经验的学生可以先看看正式课程)
  - [15. InfiniLM 不用 Nvidia 显卡能运行吗？](#15-infinilm-不用-nvidia-显卡能运行吗)
  - [16. cpp 和 rust 的题是开营后才公布吗？](#16-cpp-和-rust-的题是开营后才公布吗)
  - [17. rustling 可以用之前训练营的 rustling 替代吗？](#17-rustling-可以用之前训练营的-rustling-替代吗)
  - [18. 基础阶段的 cxx 和 rust 题目都是什么类型？算法题吗？](#18-基础阶段的-cxx-和-rust-题目都是什么类型算法题吗)
  - [19. 项目有没有整体架构图可以看看？](#19-项目有没有整体架构图可以看看)
  - [20. AI 编译器方向需要先学线代吗？](#20-ai-编译器方向需要先学线代吗)
  - [21. 如何判断 InfiniLM 是仅 CPU 运行的还是 CUDA 运行的？](#21-如何判断-infinilm-是仅-cpu-运行的还是-cuda-运行的)
  - [22. 如果 InfiniLM 运行 `cargo xtask list-turbo` 没有返回可用的 Turbo hardware 怎么办？](#22-如果-infinilm-运行-cargo-xtask-list-turbo-没有返回可用的-turbo-hardware-怎么办)
  - [23. 模型需要多少显存，2GB 好像不够](#23-模型需要多少显存2gb-好像不够)
  - [24. InfiniLM 推荐在 wsl 运行还是直接在 win 运行啊？](#24-infinilm-推荐在-wsl-运行还是直接在-win-运行啊)
  - [25. InfiniLM 是怎么检测 CUDA 的？](#25-infinilm-是怎么检测-cuda-的)
  - [26. 运行 `exam-grading` 提交评测报错找不到子模块](#26-运行-exam-grading-提交评测报错找不到子模块)
  - [27. 除了 `TinyLlama` 还有没有其他能在 InfiniLM 框架中运行的更大的模型？](#27-除了-tinyllama-还有没有其他能在-infinilm-框架中运行的更大的模型)
  - [28. 用的 mac air，配置在后续跟不跟的上呢？](#28-用的-mac-air配置在后续跟不跟的上呢)
  - [29. exam-grading：如果 initialize submodules and check for experiments 成功了，但是 test job for other submodules 跳过了是什么原因？](#29-exam-grading如果-initialize-submodules-and-check-for-experiments-成功了但是-test-job-for-other-submodules-跳过了是什么原因)
  - [30. 是否也能让 InfiniLM 支持 AMD 的显卡呢？](#30-是否也能让-infinilm-支持-amd-的显卡呢)
  - [31. Mac 的 GPU 是不是还没有支持？](#31-mac-的-gpu-是不是还没有支持)
